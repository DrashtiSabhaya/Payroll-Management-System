...
class Report
{
	public Report()
	{
		...
		try{
				PdfWriter writer=PdfWriter.getInstance(d1,new FileOutputStream("EmployeeReport.pdf"));
				d1.open();
				...
				PdfPCell c=new PdfPCell(new Paragraph("All Employee Report "));
				c.setColspan(10);
				...
				PdfPCell c5=new PdfPCell(new Paragraph("Department"));
				PdfPCell c6=new PdfPCell(new Paragraph("Total Allowance"));
			...
			PreparedStatement stmt=con.prepareStatement("select s.* from staff_info s join  ");
			//stmt.setInt(1,empid);
